    ...
    try {
      ...
      for (const img of pickedImages) {
        await new Promise<void>((resolve) => {
          ...
          xhr.onload = () => resolve();
          xhr.onerror = () => resolve(); // don't block review submission on photo error
          xhr.send(form);
        });
      }
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#f9fafb" }} edges={["top"]}>
      ...
      <View style={{ backgroundColor: "#fff", flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: "#f3f4f6" }}>
        ...
        <Text style={{ fontSize: 17, fontWeight: "800", color: "#111827" }}>Оставить отзыв</Text>
      </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={{ padding: 16, gap: 14 }} showsVerticalScrollIndicator={false}>
          ...
          {/* Rating */}
          <View style={{ backgroundColor: "#fff", borderRadius: 18, padding: 20, alignItems: "center" }}>
            <Text style={{ fontSize: 14, fontWeight: "700", color: "#111827", marginBottom: 16 }}>Ваша оценка товара</Text>
            <View style={{ flexDirection: "row", gap: 10 }}>
              {[1, 2, 3, 4, 5].map((s) => (
                <TouchableOpacity key={s} onPress={() => setRating(s)}>
                  <Star size={44} color={s <= rating ? "#f59e0b" : "#e5e7eb"} fill={s <= rating ? "#f59e0b" : "#e5e7eb"} />
                </TouchableOpacity>
              ))}
            </View>
            {rating > 0 && (
          ...
          <View style={{ backgroundColor: "#fff", borderRadius: 18, padding: 16 }}>
            <Text style={{ fontSize: 14, fontWeight: "700", color: "#111827", marginBottom: 4 }}>Добавьте фото</Text>
            <Text style={{ fontSize: 12, color: "#9ca3af", marginBottom: 12 }}>До 6 фото с разных ракурсов</Text>
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
              ...
              {pickedImages.length < 6 && (
                <TouchableOpacity
                  onPress={pickImages}
                  style={{ width: 80, height: 80, borderRadius: 12, backgroundColor: "#f9fafb", alignItems: "center", justifyContent: "center", borderWidth: 1.5, borderColor: "#e5e7eb", borderStyle: "dashed" }}
                >
                  <Camera size={24} color="#9ca3af" />
                  <Text style={{ fontSize: 10, color: "#9ca3af", marginTop: 4 }}>Фото</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
          ...
          {/* Comment */}
          <View style={{ backgroundColor: "#fff", borderRadius: 18, padding: 16 }}>
            <Text style={{ fontSize: 14, fontWeight: "700", color: "#111827", marginBottom: 12 }}>Комментарий</Text>
            <TextInput
              value={text}
              onChangeText={setText}
              placeholder="Расскажите о качестве, доставке, упаковке..."
              multiline
              style={{ backgroundColor: "#f9fafb", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, color: "#374151", minHeight: 110, textAlignVertical: "top" }}
            />
          </View>
          ...
          <TouchableOpacity
            ...
            style={{ backgroundColor: rating === 0 ? "#e5e7eb" : "#111827", paddingVertical: 16, borderRadius: 16, alignItems: "center" }}
          >
            {submitting ? (
              <ActivityIndicator color="#fff" />
            ) : (
              <Text style={{ color: rating === 0 ? "#9ca3af" : "#fff", fontWeight: "700", fontSize: 15 }}>Отправить отзыв</Text>
            )}
          </TouchableOpacity>
