...
import { ArrowLeft, X, Camera, Plus, Trash2, Search, ChevronRight, Check, Eye, TrendingUp, AlertTriangle } from "lucide-react-native";
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Find which root the product's category belongs to
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ backgroundColor: "#fff", borderRadius: 20, padding: 16, gap: 12 }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 10 }}>
            ...
            {newPhotos.map((p, i) => (
              <View key={`new-${i}`} style={{ width: 84, height: 84, borderRadius: 16, overflow: "hidden", borderWidth: 2, borderColor: "#4ade80" }}>
                <Image source={{ uri: p.uri }} style={{ width: 84, height: 84 }} contentFit="cover" />
                <TouchableOpacity onPress={() => setNewPhotos((prev) => prev.filter((_, j) => j !== i))} style={{ position: "absolute", top: 4, right: 4, width: 22, height: 22, backgroundColor: "rgba(0,0,0,0.5)", borderRadius: 11, alignItems: "center", justifyContent: "center" }}>
                  <X size={12} color="white" />
                </TouchableOpacity>
