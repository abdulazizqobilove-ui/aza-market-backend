...
function TabIcon({ icon, focused, count }: { icon: (color: string) => React.ReactNode; focused: boolean; count?: number }) {
  ...
  return (
    <View style={{ alignItems: "center", justifyContent: "center", marginTop: 6 }}>
      <View style={{ width: 48, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center", position: "relative" }}>
        {icon(color)}
...
function SwipeTabsWrapper({ children }: { children: React.ReactNode }) {
  ...
  const gesture = Gesture.Pan()
    ...
    .onEnd((e) => {
      ...
      const goNext = (e.translationX < -60 || e.velocityX < -500) && idx < TAB_ROUTES.length - 1;
      const goPrev = (e.translationX > 60  || e.velocityX >  500) && idx > 0;
      if (goNext) runOnJS(navigate)(TAB_ROUTES[idx + 1]);
      else if (goPrev) runOnJS(navigate)(TAB_ROUTES[idx - 1]);
    });
    ...
    <SwipeTabsWrapper>
      <Tabs screenOptions={tabOptions}>
        <Tabs.Screen name="index"
          options={{ tabBarIcon: ({ focused }) => <TabIcon focused={focused} icon={(c) => <Home size={24} color={c} strokeWidth={focused ? 2.5 : 1.8} />} /> }}
        />
        <Tabs.Screen name="catalog"
          options={{ tabBarIcon: ({ focused }) => <TabIcon focused={focused} icon={(c) => <Menu size={24} color={c} strokeWidth={focused ? 2.5 : 1.8} />} /> }}
        />
        <Tabs.Screen name="cart"
          options={{ tabBarIcon: ({ focused }) => <TabIcon focused={focused} count={cartCount} icon={(c) => <ShoppingCart size={24} color={c} strokeWidth={focused ? 2.5 : 1.8} />} /> }}
        />
        ...
        <Tabs.Screen name="favorites"
          options={{ tabBarIcon: ({ focused }) => <TabIcon focused={focused} icon={(c) => <Heart size={24} color={c} strokeWidth={focused ? 2.5 : 1.8} fill={focused ? c : "none"} />} /> }}
        />
