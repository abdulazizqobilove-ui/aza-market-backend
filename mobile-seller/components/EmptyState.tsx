...
export function EmptyState({ emoji, title, subtitle, actionLabel, onAction }: Props) {
  ...
  return (
    <View style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 40, paddingVertical: 60, gap: 12 }}>
      <View style={{ width: 96, height: 96, borderRadius: 48, backgroundColor: c.iconBg, alignItems: "center", justifyContent: "center", marginBottom: 8 }}>
        <Text style={{ fontSize: 44 }}>{emoji}</Text>
      </View>
      <Text style={{ fontSize: 18, fontWeight: "800", color: c.text, textAlign: "center" }}>{title}</Text>
      {subtitle && (
        <Text style={{ fontSize: 14, color: c.textMuted, textAlign: "center", lineHeight: 20 }}>{subtitle}</Text>
      )}
