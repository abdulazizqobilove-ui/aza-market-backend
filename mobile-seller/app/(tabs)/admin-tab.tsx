                ...
                <View style={{ backgroundColor: "#f5f3ff", borderRadius: 16, padding: 14, gap: 12 }}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                      ...
                      <View>
                        <Text style={{ fontSize: 14, fontWeight: "800", color: "#111827" }}>Главный экран</Text>
                        <Text style={{ fontSize: 12, color: "#9ca3af", marginTop: 1 }}>
                          {banners.filter(b => !b.link_url).length > 0
                            ? `${banners.filter(b => !b.link_url).length} баннер(а)`
                            : "Нет баннеров"}
                        </Text>
                      </View>
                    </View>
                    <TouchableOpacity
                      onPress={() => setBannerFormState({ visible: true, existing: null, initialLinkUrl: null })}
                      style={{ flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: P, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8 }}
                    >
                      <Plus size={13} color="#fff" />
                      <Text style={{ fontSize: 12, fontWeight: "700", color: "#fff" }}>Добавить</Text>
                    </TouchableOpacity>
                  </View>

                  {/* List of main screen banners */}
                  {banners.filter(b => !b.link_url).length === 0 ? (
                    <Text style={{ fontSize: 12, color: "#9ca3af", textAlign: "center", paddingVertical: 4 }}>Нет баннеров — нажмите «Добавить»</Text>
                  ) : banners.filter(b => !b.link_url).map((b) => (
                    <View key={b.id} style={{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#fff", borderRadius: 12, padding: 10 }}>
                      {/* Mini preview */}
                      <View style={{ width: 60, height: 38, borderRadius: 9, backgroundColor: b.bg_color, overflow: "hidden" }}>
                        {b.image_url
                          ? <Image source={{ uri: imgUrl(b.image_url) ?? "" }} style={{ width: 60, height: 38 }} contentFit="cover" />
                          : <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                              <Text style={{ fontSize: 18 }}>{b.emoji || "🖼️"}</Text>
                            </View>
                        }
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 13, fontWeight: "700", color: "#111827" }} numberOfLines={1}>{b.title || "Без названия"}</Text>
                        <Text style={{ fontSize: 11, color: b.is_active ? "#16a34a" : "#9ca3af", marginTop: 1 }}>
                          {b.is_active ? "● Активен" : "○ Отключён"}
                        </Text>
                      </View>
                      <TouchableOpacity
                        onPress={() => setBannerFormState({ visible: true, existing: b })}
                        style={{ width: 32, height: 32, borderRadius: 9, backgroundColor: "#f0f9ff", alignItems: "center", justifyContent: "center" }}
                      >
                      ...
                      <TouchableOpacity
                        ...
                        style={{ width: 32, height: 32, borderRadius: 9, backgroundColor: "#fef2f2", alignItems: "center", justifyContent: "center" }}
                      >
