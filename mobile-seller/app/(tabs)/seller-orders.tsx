              ...
              <View style={{ backgroundColor: c.card, borderRadius: 18, overflow: "hidden" }}>
                ...
                {isExp && (
                  <View style={{ paddingHorizontal: 16, paddingTop: 12, paddingBottom: 12, gap: 10, borderTopWidth: 0.5, borderTopColor: c.border, marginTop: 10 }}>
                    ...
                    {(order.payment_method === "cash" || order.payment_method === "on_delivery") && !order.is_paid && (
                      <TouchableOpacity
                        onPress={() => markPaid(order.id)}
                        disabled={isUpdating}
                        style={{ backgroundColor: "#dcfce7", borderRadius: 14, paddingVertical: 12, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 8 }}
                      >
                        {isUpdating ? (
                          ...
                          <>
                            ...
                            <Text style={{ fontSize: 13, fontWeight: "700", color: "#16a34a" }}>Подтвердить оплату наличными</Text>
                          </>
                        )}
                      </TouchableOpacity>
                    )}
