...
export default function PrivacyScreen() {
  ...
  const H1 = ({ children }: { children: string }) => (
    <Text style={{ fontSize: 20, fontWeight: "900", color: c.text, marginTop: 24, marginBottom: 8 }}>{children}</Text>
  );
  const H2 = ({ children }: { children: string }) => (
    <Text style={{ fontSize: 15, fontWeight: "700", color: c.textSub, marginTop: 16, marginBottom: 6 }}>{children}</Text>
  );
  const P = ({ children }: { children: React.ReactNode }) => (
    <Text style={{ fontSize: 14, color: c.textSub, lineHeight: 22 }}>{children}</Text>
  );
  const Li = ({ children }: { children: string }) => (
    <View style={{ flexDirection: "row", gap: 8, marginBottom: 4 }}>
      <Text style={{ fontSize: 14, color: "#8B5CF6", marginTop: 4 }}>•</Text>
      <Text style={{ flex: 1, fontSize: 14, color: c.textSub, lineHeight: 22 }}>{children}</Text>
    </View>
  );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.card }} edges={["top"]}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: c.border }}>
        <TouchableOpacity onPress={() => router.back()} style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: c.iconBg, alignItems: "center", justifyContent: "center" }}>
          <ArrowLeft size={18} color={c.textSub} />
        </TouchableOpacity>
        <Text style={{ fontSize: 17, fontWeight: "800", color: c.text }}>Политика конфиденциальности</Text>
      </View>

      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 48 }}>
        <Text style={{ fontSize: 13, color: c.textMuted, marginBottom: 4 }}>Дата вступления в силу: 26 мая 2026 г.</Text>
        ...
        <View style={{ marginTop: 32, backgroundColor: c.iconBg, borderRadius: 14, padding: 16 }}>
